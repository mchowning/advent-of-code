...
import           Turtle.Pattern   (Pattern, choice, match, newline, sepBy)
...
input :: IO [Direction]
input = head . match directionPattern <$> TIO.readFile "src/input_day11.txt"
...
directionPattern :: Pattern [Direction]
directionPattern = do
  d <- choice ["n", "ne", "e", "se", "s", "sw", "w", "nw"] `sepBy` "," <* newline
  return $ map makeDirection d
  where
    makeDirection =
      \case
        "n" -> N
        ...
        "sw" -> SW
        "nw" -> NW
        _ -> error "no other direction value is permitted"
...
---------------------------------------------------------------------
tests :: IO ()
tests = defaultMain $ testGroup "Day 11 tests"
  [ moveTests
  , part1AlgoTests
  ]
  where
    moveTests = testGroup "move"
    ...
    part1AlgoTests = testGroup "part 1 algorithm"
