...
route = route' (0,0)
  where
    route' :: (Int, Int) -> [Move] -> [(Int, Int)]
    route' _ [] = []
    route' start (m:ms) =
      let end = findEnd start m
          newPositions = tail (travel start end)
      in newPositions <> route' end ms

    travel :: (Int, Int) -> (Int, Int) -> [(Int, Int)]
    travel (x1,y1) (x2,y2) = (,) <$> safeRange x1 x2 <*> safeRange y1 y2
...
findEnd :: (Int, Int) -> Move -> (Int, Int)
findEnd (x,y) (Move direction distance) = case direction of
  Up -> (x, y + distance)
  Right -> (x + distance, y)
  Down -> (x, y - distance)
  Left -> (x - distance, y)
...
part2' :: ([Move], [Move]) -> Int
part2' ms =
  let (coordDistance1, coordDistance2) = join bimap (shortestDistanceToEachCoord . zip [1..] . route) ms
...
parser = do
  ...
  return (moves1, moves2)
    where moves = move `sepBy1` char ','

move :: Parser Move
move = do
  rawDirection <- satisfy (`elem` ['U', 'R', 'D', 'L'])
  distance <- decimal
  let direction = case rawDirection of
        'U' -> Up
        'R' -> Right
        'D' -> Down
        'L' -> Left
...
test :: IO ()
test = runTestCases (TestCase "part 1" 721 part1)
       (TestCase "part 2" 7388 part2)
