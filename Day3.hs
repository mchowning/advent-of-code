...
locations = scanl move (0,0)
  where
      ...
      move (x,y) dir = case dir of '^' -> (x,y+1)
                                   '>' -> (x+1,y)
                                   'v' -> (x,y-1)
                                   '<' -> (x-1,y)
                                   _   -> undefined
...
tests = runTestTT $ TestList [ inputTests 
                             , locationsTests
                             , allLocationsForPairTests
                             ]
    where
        inputTests :: Test
        inputTests = TestList [ result1 ~?= 2565
                              , result2 ~?= 2639
                              ]

        locationsTests :: Test
        locationsTests = let output = locations ">^<v" in
                         TestList [ length output ~?= 5
                                  , hasNTuples output (0,0) 2
                                  , hasNTuples output (0,1) 1
                                  , hasNTuples output (1,1) 1
                                  , hasNTuples output (1,0) 1
                                  ]

        allLocationsForPairTests :: Test
        allLocationsForPairTests = let output = allLocationsForPair "^v^v" in
                                   TestList [ length output ~?= 6
                                            , (length . filter (==(0,0)) $ output) ~?= 2
                                            , hasNTuples output (0,1) 1
                                            , hasNTuples output (0,2) 1
                                            , hasNTuples output (0,-1) 1
                                            , hasNTuples output (0,-2) 1
                                            ]

        hasNTuples :: [(Int,Int)] -> (Int,Int) -> Int -> Test
        hasNTuples ls tup n = (length . filter (== tup)) ls ~?= n
